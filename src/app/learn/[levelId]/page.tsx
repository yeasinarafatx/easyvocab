...
import level01 from "@/data/beginner/level_01.json";
...
export default function LearnLevelPage() {
  ...
  const hasMistake = normalizedTyped
    .split("")
    .some((typedChar, index) => typedChar !== expectedWord[index]);
  ...
  const speakWord = () => {
    if (typeof window === "undefined" || !("speechSynthesis" in window) || isCompleted || !activeWord) {
      return;
    }
  ...
  useEffect(() => {
    speakWord();
  }, [currentIndex, speechRate, selectedVoiceURI, activeWord]);
  ...
  useEffect(() => {
    if (!examMode || !isCompleted || showSuccessModal) {
      return;
    }
  ...
  }, [examMode, isCompleted, levelId, showSuccessModal]);

  const handleNext = () => {
    if (isCompleted || !activeWord) {
      return;
    }
        ...
        <section className="rounded-3xl border border-white/15 bg-white/10 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
          ...
          <div className="mt-4 flex justify-end">
            <div className="flex flex-wrap justify-end gap-2">
              <button
                type="button"
                onClick={() => setSpeechRate((prev) => (prev === 1 ? 0.7 : 1))}
                className="rounded-lg border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/20"
              >
              ...
              <button
                ...
                className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${
                  examMode
                    ? "border border-rose-300/40 bg-rose-300/20 text-rose-100 hover:bg-rose-300/30"
          ...
          {examMode ? (
            <p className="mt-3 text-sm text-amber-200">
              Exam mode চালু: একটি ভুল হলেই আবার ১ নম্বর শব্দ থেকে শুরু হবে।
            </p>
          ) : null}

          {restartNotice ? (
            <p className="mt-2 text-sm font-semibold text-rose-300">
              ভুল হয়েছে। Exam restart: আবার ১ নম্বর শব্দ থেকে শুরু করুন।
            </p>
          ) : null}
          ...
          {!isCompleted ? (
            <>
              <div className="mt-7 rounded-2xl border border-white/15 bg-white/10 p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <span className="rounded-full border border-cyan-200/30 bg-cyan-200/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-cyan-100">
                        {activeWord.pos}
                      </span>
                      <span className="text-sm text-slate-300">{activeWord.phonetic}</span>
                    </div>
                  </div>
                ...
                <p className="mt-2 text-sm text-slate-300">{activeWord.definition}</p>
                <p className="mt-2 text-sm italic text-slate-300">
                  {examMode
                    ? activeWord.example.replace(
                        new RegExp(`\\b${activeWord.word}\\b`, "gi"),
                        "_".repeat(activeWord.word.length),
                      )
                    : activeWord.example}
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {activeWord.word.split("").map((char: string, index: number) => {
                    ...
                    return (
                      <div
                        key={`${char}-${index}`}
                        className={`flex h-12 w-12 items-center justify-center rounded-xl border text-xl font-bold uppercase ${boxClass}`}
                      >
                        {typedChar ?? ""}
                      </div>
                    );
              ...
              <div className="mt-6 flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  onClick={handleNext}
                  disabled={!isCorrect}
                  className="rounded-xl bg-gradient-to-r from-cyan-300 to-emerald-300 px-5 py-2.5 text-sm font-bold text-[#0f0f1a] transition enabled:hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Next
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-md rounded-2xl border border-white/20 bg-[#16162a]/90 p-6 text-center shadow-2xl backdrop-blur-xl">
            <h3 className="text-2xl font-extrabold text-emerald-200">
              অভিনন্দন! পরের লেভেল আনলক হয়েছে 🎉
            </h3>
            <p className="mt-3 text-sm text-slate-300">আপনি Exam Mode সফলভাবে পাস করেছেন।</p>
            <Link
              href="/dashboard"
              className="mt-6 inline-flex rounded-xl bg-gradient-to-r from-cyan-300 to-emerald-300 px-5 py-2.5 text-sm font-bold text-[#0f0f1a] transition hover:brightness-110"
            >
